...
public class CheckTodosCretedept {
	Properties deptProp;
	DbUtil dbUtil;
	DB db;
	...
	public CheckTodosCretedept() {
		try {
			...
			BufferedWriter bw = bw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream( new File("c:/todos_dept_" + file + ".sql")), "UTF-8"));
			
			String sql = "select distinct TaskCreator from todos where TaskCreator!='' ";
			List<String> keys2 = getValueList(sql, "TaskCreator");
			System.out.println(keys2.size());
			
			for(String key : keys2) {
				sql = "select Dept_id from employee where flowkey ='" + key + "'";
				String dept = getFieldValue(sql, "Dept_id");
				System.out.println(key + " " + dept);
				
				bw.write("update todos set CreateDept = '" + dept + "' where TaskCreator='" + key + "';" + Cnstnts.NEW_LINE);
			}
...
//			db = null;
	...
	public static void  main(String args[]) {
		CheckTodosCretedept test = new CheckTodosCretedept();
	}
