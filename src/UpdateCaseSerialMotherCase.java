...
public class UpdateCaseSerialMotherCase {
	Properties deptProp;
	DbUtil dbUtil;
	DB db;
	...
	public UpdateCaseSerialMotherCase() {
		try {
			dbUtil = new DbUtil();
			db=new DB();
			int line = 0;
			...
			BufferedWriter bw = bw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream( new File("c:/updateSerialMotherCase" + file + ".sql")), "UTF-8"));
			...
			for(int i=0;i<cass.length;i++) {
				...
				bw.write("update " + cass[i] + " set SerialMotherCase='';" + Cnstnts.NEW_LINE);
				
				String sql = "SELECT DISTINCT SUBSTR( caseno, 1, 8 ) as caseno FROM  `" + cass[i] + "`  WHERE caseno !=  '' AND LENGTH( caseno ) >=8";
				List<String> headers = getValueList(sql, "caseno");
				for(String header : headers) {
					sql = "SELECT flowkey, caseno FROM  `" + cass[i] + "`  WHERE caseno LIKE  '" + header + "%' AND " + createdates[i] + " !=  '' ORDER BY " + createdates[i] + ", flowkey";
//					System.out.println(sql);
					List<List<Field>> nos = getFieldsList(sql, new String[]{"flowkey", "caseno"});
					if(nos != null && !nos.isEmpty()) {
						String firstKey = nos.get(0).get(0).getValue();
						String firstNo = nos.get(0).get(1).getValue();
						bw.write("update " + cass[i] + " set SerialMotherCase='" + firstNo + "' where caseno LIKE  '" + header + "%' AND flowkey !=  '" + firstKey + "';" + Cnstnts.NEW_LINE);
					}
					
					sql = "SELECT flowkey, caseno FROM  `" + cass[i] + "`  WHERE caseno LIKE  '" + header + "%' AND " + applydates[i] + " !=  '' AND " + createdates[i] + " =  ''  ORDER BY " + applydates[i] + ", flowkey";
//					System.out.println(sql);
					nos = getFieldsList(sql, new String[]{"flowkey", "caseno"});
					if(nos != null && !nos.isEmpty()) {
						String firstKey = nos.get(0).get(0).getValue();
						String firstNo = nos.get(0).get(1).getValue();
						//bw.write("update " + cass[i] + " set SerialMotherCase='" + firstNo + "' where caseno LIKE  '" + header + "%' AND flowkey !=  '" + firstKey + "';" + Cnstnts.NEW_LINE);
						bw.write("update " + cass[i] + " set SerialMotherCase='" + firstNo + "' where caseno LIKE  '" + header + "%' ;" + Cnstnts.NEW_LINE);
					}
				}
...
//			db = null;
	...
	public static void  main(String args[]) {
		UpdateCaseSerialMotherCase test = new UpdateCaseSerialMotherCase();
	}
