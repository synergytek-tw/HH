...
public class CreateQuotationInvoiceTitle {
	Properties deptProp;
	DbUtil dbUtil;
	DB db;
	...
	public CreateQuotationInvoiceTitle() {
		try {
			dbUtil = new DbUtil();
			db=new DB();
			int line = 0;
			...
			BufferedWriter bw = bw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream( new File("c:/quotation2" + file + ".sql")), "UTF-8"));
			
			String sql = "select distinct client from quotation  where invoicetitle = '' ";
...
//					}
					...
					sql = "select a.flowkey from invtitle a " +
		            		"where a.parentid = '" + client + "' " +
		            		"order by a.flowkey";
					String names = getFieldValue(sql, "flowkey");
							
					bw.write("update quotation set invoicetitle = '" + names + "' " +
							"where client = '" + client + "' " +
...
//			db = null;
	...
	public static void  main(String args[]) {
		CreateQuotationInvoiceTitle test = new CreateQuotationInvoiceTitle();
	}
