...
public class UpdateLoginfoSrcFlowKey {
	Properties deptProp;
	DbUtil dbUtil;
	DB db;
	...
	public UpdateLoginfoSrcFlowKey() {
		try {
			dbUtil = new DbUtil();
			db=new DB();
			int line = 0;
			...
			BufferedWriter bw = bw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream( new File("c:/UpdateLoginfoSrcFlowKey" + file + ".sql")), "UTF-8"));
			...
			for(int i=0;i<cass.length;i++) {
				...
				for(String header : headers) {
					...
					if(casenos != null && !casenos.isEmpty()) {
						bw.write("update loginfo set SrcFlowKey='" + casenos.get(0) + "' where SrcFlowKey = '" + header + "';" + Cnstnts.NEW_LINE);
					}
				}
...
//			db = null;
	...
	public static void  main(String args[]) {
		UpdateLoginfoSrcFlowKey test = new UpdateLoginfoSrcFlowKey();
	}
