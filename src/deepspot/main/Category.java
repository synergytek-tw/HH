...
public class Category extends MasterBean {
	...
	public void init() {
		...
		this.setKeepDataProcessFlag("true");
		...
		this.Memo.setSearchCondition(Cnstnts.LIKE);
	...
	public List<SelectItem> getChildList(Field field, boolean withKey) throws Exception {
		String whereStr = Cnstnts.EMPTY_STRING;
		if(field!=null){
			whereStr = " where ParentId='"+field.getName()+"'";
		}
		String SQL="select * from "+this.getTableName()+
					whereStr+" order by FlowKey";
		return this.getDbUtil().getSelectItemList(SQL, this.Names, withKey);
	}
	
	public List<SelectItem> getChildList(Field field, List removeList) throws Exception {
        String whereStr = Cnstnts.EMPTY_STRING;
        if(field!=null){
            whereStr = " where ParentId='"+field.getName()+"'";
        }
        String SQL="select * from "+this.getTableName()+
                    whereStr+" order by FlowKey";
        List<SelectItem> result = this.getDbUtil().getSelectItemList(SQL, this.Names);
        for(int i=result.size()-1;i>=0;i--) {
            SelectItem item = result.get(i);
            if(removeList.contains(item.getKey())) {
                result.remove(i);
            }
        }
        return result;
	...
	public List<SelectItem> getChildList(String parentId) throws Exception {
		...
		return getChildList(parentId, true);
	}
	
	public List<SelectItem> getChildList(String parentId, boolean withKey) throws Exception {
		String whereStr = Cnstnts.EMPTY_STRING;
		if(parentId!=null){
			whereStr = " where ParentId='"+parentId+"'";
		}
		String SQL="select * from "+this.getTableName()+ whereStr+" order by FlowKey";
		return this.getDbUtil().getSelectItemList(SQL, this.Names, withKey);
	}
	...
	public List<SelectItem> getChildList(String parentId, List removeList) throws Exception {
        String whereStr = Cnstnts.EMPTY_STRING;
        if(parentId!=null){
            whereStr = " where ParentId='"+parentId+"'";
        }
	...
	public List<SelectItem> getGroupList(Field field) throws Exception {
		...
		return getGroupList(field.getName());
	}
	
	public List<SelectItem> getGroupList(String field) throws Exception {
		String whereStr = Cnstnts.EMPTY_STRING;
		if(field!=null){
			whereStr = " where CategoryGroup='"+field+"'";
		}
		String SQL="select * from "+this.getTableName()+
					whereStr+" order by FlowKey";
		return this.getDbUtil().getSelectItemList(SQL, this.Names);
	...
	public String printSearchTable() throws Exception {
		...
		if (this.onSearchMode()) {
			...
			List<SelectItem> items = this.getMenuList();
			items.add(0, new SelectItem("Currency", "Currency-幣別"));
			
			this.ParentId.setHtmlType(Cnstnts.HTML_TAG_SELECT_ONE);
			this.ParentId.setSelectList(items);
			rtnString += this.printHtml(this.ParentId);

			this.Names.setHtmlType(Cnstnts.HTML_TAG_TEXT);
			this.Names.setLength(200);
			this.Names.setSize(80);
			rtnString += this.printHtml(this.Names);
			
			this.Memo.setHtmlType(Cnstnts.HTML_TAG_TEXT);
			this.Memo.setLength(200);
			this.Memo.setSize(80);
	...
	public String printMaintainTable() throws Exception {
		...
		if (this.onMaintainMode()) {
			...
			this.ParentId.setAnotherField(this.CategoryGroup);
			rtnString += this.printHtml(this.ParentId);
			
			this.CategoryFlag.setHtmlType(Cnstnts.HTML_TAG_CHECKBOX);
			rtnString += this.printHtml(this.CategoryFlag);
...
//			rtnString += this.printHtml(this.Controler);
	...
	public void setSearchListPrpts() {
		...
		this.Rate.setListWidth("15%");

		Field[] showField = {this.FlowKey, this.Names, this.ParentId, this.Rate};
	...
	public List<Object[]> getSearchList() throws Exception {
		...
		whereStr = Util.addWhereSQL(whereStr, this.Memo);
	...
	public boolean checkInsertError() throws Exception {
		if(super.checkInsertError()){
			...
			/*
			[嚙磊嚙踝蕭 04:35:37] Janny Kuo: 嚙緬嚙踝蕭嚙踝蕭嚙瞌嚙瞎 嚙緹嚙褒設嚙緩嚙踝蕭 嚙編嚙磕嚙踝蕭嚙�嚙綞嚙編嚙踝蕭嚙誼核代嚙碼嚙瞌嚙稻嚙踝蕭嚙踝蕭嚙踝蕭嚙踝蕭嚙複，
			嚙踝蕭嚙踝蕭嚙豌要嚙踝蕭警嚙踝蕭"嚙諉代嚙碼嚙瞑嚙緬嚙踝蕭嚙踝蕭嚙瞌嚙諄程嚙褒設嚙緩嚙褕既嚙踝蕭嚙踝蕭嚙諍哨蕭嚙複請更正恬蕭b嚙綞嚙編"
			        	 */
			        	if(!Util.isEmpty(this.getFlowKey())){
			        		String sql = "select count(1) as Count from patentmatterconfig where flowkey='" + this.getFlowKey() + "'";
			        		int count = Integer.parseInt(this.getDbUtil().getFieldValue(sql, "Count"));
			        		if(count > 0) {
			        			this.setErrorMessage(this.getUserInfo().getMsg("jsp.PatentMatterConfig.FlowkeyError"));
			        			return true;
			        		}
			        	}
			return false;
	...
	public SelectItem getChildItem(String parentId, String flowKey) throws Exception {
		String whereStr = Cnstnts.EMPTY_STRING;
		if(parentId!=null){
			whereStr = " where ParentId='"+parentId+"' and FlowKey='" + flowKey + "'";
		}
		String SQL="select * from "+this.getTableName()+
						whereStr+" order by FlowKey";
		List<SelectItem> result = this.getDbUtil().getSelectItemList(SQL, this.Names);
	...
	public HashMap getRateMap() throws Exception {
		HashMap map = new HashMap<String, String>();
		
		String sql = "select flowkey, rate from category where parentid = 'Currency'";
		List<Object[]> result = this.getDbUtil().getList(sql, new String[]{"rate" });
	...
	public boolean insert() throws Exception {
		if(super.insert()) {
			Config.updatesProperty("Category_Names", this.getFlowKey(), this.getNames());
	...
	public boolean delete() throws Exception {
		if(super.delete()) {
			Config.removeProperty("Category_Names", this.getFlowKey());
