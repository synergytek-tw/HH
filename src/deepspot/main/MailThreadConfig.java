...
import deepspot.mail.SMTPAuthenticator;
...
public class MailThreadConfig {
  ...
  String TestAddress="";
  public void setTestAddress(String TestAddress){this.TestAddress=TestAddress;}
  ...
  public String getSmtpPortString(){return SmtpPort+"";}

  String SendTestMail="";
  public void setSendTestMail(String SendTestMail){this.SendTestMail=SendTestMail;}
  public String getSendTestMail(){return SendTestMail;}
  ...
  public void getAttribute() {
    ...
    try {
      ...
      if(rs.next()) {
        ...
        this.SmtpPort=rs.getInt("SmtpPort");
        this.SecurityType=rs.getString("SecurityType");
        this.TestAddress=rs.getString("TestAddress");
  ...
  public Iterator<MailThreadConfig> getList() {
    ...
    try {
      ...
      while(rs.next()) {
        ...
        obj.setSecurityType(rs.getString("SecurityType"));
        obj.setTestAddress(rs.getString("TestAddress"));
        obj.setExchange(rs.getString("Exchange"));
        v.add(obj);
      }
      rs.close();
      db.close();
  ...
  public void insert() {
  	log(KeyValue);
    DB db=null;
    String sqlStr="insert into MailThreadConfig(KeyValue,ExecuteStatus,LastExecuteTime,NextExecuteTime,DelayTime,MailPeriod,Host,EmailAddress,EmailAccount,Password,Authority,TurnOn,TestAddress,SecurityType,SmtpPort,Exchange) values('"+KeyValue+"','"+ExecuteStatus+"','"+LastExecuteTime+"','"+NextExecuteTime+"','"+DelayTime+"','"+MailPeriod+"','"+Host+"','"+EmailAddress+"','"+EmailAccount+"','"+Password+"','"+Authority+"','"+TurnOn+"','"+TestAddress+"','" + SecurityType + "', " + SmtpPort + ", '" + Exchange + "')";
    try {
      db=new DB();
  ...
  public boolean dbExcute(String db_type,String users) throws Exception {
    ...
    if(Util.compareTo(this.getSendTestMail(), "true")) {
    	if(Util.isEmpty(TestAddress) || !Util.chkEmail(TestAddress)) {
    		this.Msg=Util.getMsg("jsp.MailThreadConfig.TestAddressError",users);
    	}
    	else {
    		sendTestMail(users);
    		this.Msg=Util.getMsg("jsp.MailThreadConfig.TestFinish",users);
    	}
    }
  ...
  private void sendTestMail(String users) {
		try {
			Properties props = System.getProperties();
			System.out.println("mail.smtp.host="+this.getHost());
			System.out.println("mail.smtp.port="+this.getSmtpPort());
			
			
			props.put("mail.smtp.host", this.getHost());
			if(Util.compareTo(this.getAuthority(), "Y")){
				props.put("mail.smtp.auth", "true");
			}else{
				props.put("mail.smtp.auth", "false");
			}
			if(!Util.isEmpty(this.getSmtpPort()+"")){
				props.put("mail.smtp.port", this.getSmtpPort()+"");
			}else{
				props.put("mail.smtp.port", "25");
			}
			
			if(Util.compareTo(this.getSecurityType(), "TLS")) {
				props.put("mail.smtp.starttls.enable", "true");  
			}else{
				props.put("mail.smtp.starttls.enable", "false");
			}
			if(Util.compareTo(this.getSecurityType(), "SSL")) {
			...
			String account = "";
			if(Util.compareTo(this.getExchange(), "Y")){
				String[] tempAccount = null;
				if(this.getEmailAccount().indexOf("@") != -1){
					tempAccount = this.getEmailAccount().split("@");
					if(!Util.isEmpty(tempAccount[0])){
						account = tempAccount[0];
					}
				}else{
					account = this.getEmailAccount();
				}
			}else{
				account = this.getEmailAccount();
			}
//			Authenticator auth = new SMTPAuthenticator(this.getEmailAccount(), this.getPassword());
			Authenticator auth = new SMTPAuthenticator(account, this.getPassword());

			Session session = Session.getInstance(props, auth);
			//session.setDebug(true);
			MimeMessage message = new MimeMessage(session);
			message.addRecipients(Message.RecipientType.TO, this.getTestAddress());

			InternetAddress from = new InternetAddress(this.getEmailAccount());
			from.setPersonal(this.getEmailAddress());
			...
			messageBodyPart.setContent(Util.getMsg("jsp.MailThreadConfig.TestAddressContent",users), "text/html; charset=utf-8");
			messageBodyPart.setHeader("Content-Transfer-Encoding", "quoted-printable");
			multipart.addBodyPart(messageBodyPart);
			message.setContent(multipart);
			
			//message.setSubject(Util.getMsg("jsp.MailThreadConfig.TestAddressTitle",users));
			message.setSubject(MimeUtility.encodeText(Util.getMsg("jsp.MailThreadConfig.TestAddressTitle",users),"UTF-8","B"));

			Transport.send(message);
			
			System.out.println("sendTestMail success");
		} catch (Exception e) {
			e.printStackTrace();
		}
  }
