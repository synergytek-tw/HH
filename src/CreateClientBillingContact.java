...
public class CreateClientBillingContact {
	Properties deptProp;
	DbUtil dbUtil;
	DB db;
	...
	public CreateClientBillingContact() {
		try {
			dbUtil = new DbUtil();
			db=new DB();
			int line = 0;
			...
			BufferedWriter bw = bw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream( new File("c:/quotation" + file + ".sql")), "UTF-8"));
			
			String sql = "select distinct client from quotation  where BillingContact = '' ";
			List<List<Field>> keys2 = getFieldsList(sql, new String[]{"client"});
...
//					}
					...
					sql = "select a.flowkey from applicant a " +
		            		"where a.ShowInClient = 'Y' " +
		            		"and a.Client = '" + client + "' " +
		            		"and a.ApplicantKind = 'CLK02' " +
		            		"order by flowkey";
					String names = getFieldValue(sql, "flowkey");
							
					bw.write("update quotation set BillingContact = '" + names + "' " +
							"where client = '" + client + "' " +
							"and BillingContact = '';" + Cnstnts.NEW_LINE);
...
//			db = null;
	...
	public static void  main(String args[]) {
		CreateClientBillingContact test = new CreateClientBillingContact();
	}
