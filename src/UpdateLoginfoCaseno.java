...
public class UpdateLoginfoCaseno {
	Properties deptProp;
	DbUtil dbUtil;
	DB db;
	...
	public UpdateLoginfoCaseno() {
		try {
			...
			BufferedWriter bw = bw = new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream( new File("c:/UpdateLoginfoCaseno" + file + ".sql")), "UTF-8"));
			
			
			String[] cass = {"SalesOrder", "Invoice"};
			System.out.println("start in " + new Date());
			for(int i=0;i<cass.length;i++) {
				System.out.println("start do " + cass[i]);
				
				String sql = "SELECT DISTINCT SrcFlowKey FROM loginfo where CreateTable='" + cass[i] + "'";
				List<String> headers = getValueList(sql, "SrcFlowKey");
				for(String header : headers) {
					sql = "SELECT caseno FROM  `" + cass[i] + "`  WHERE flowkey = '" + header + "' ";

					List<String> casenos = dbUtil.getValueList(sql, "caseno");
					if(casenos != null && !casenos.isEmpty()) {
						bw.write("update loginfo set caseno='" + casenos.get(0) + "' where SrcFlowKey = '" + header + "';" + Cnstnts.NEW_LINE);
					}
				}
...
//			db = null;
	...
	public static void  main(String args[]) {
		UpdateLoginfoCaseno test = new UpdateLoginfoCaseno();
	}
