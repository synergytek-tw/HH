...
public class DraftMasterBean {
	...
	public String getSubmitBtn() throws Exception {
		...
		if (this.canSubmit()) {
			...
			rtnString += Html.getSubmitBtn(this.userInfo, this.DbActionMode);
		}
		return rtnString;
	}
			...
			if (Cnstnts.SIGN.equals(this.DbActionMode)) {
				...
				if (this.insert()) {
					...
					if(Cnstnts.TRUE_FLAG.equals(this.getKeepProcessFlag())){
						this.ForwardPage = this.getTableName()+".jsp?_pageActionMode=maintain"+
																	"&_clearFlag=true";
					} else {
						this.ForwardPage = this.getTableName()+".jsp?_pageActionMode=search"+
																	"&doSearchFlag=true";
					}
				} else {
	...
	protected String getLogSQL(HashMap<?, ?> beforeMap) throws Exception {
		...
		if(this.userInfo==null){
			...
			language = this.userInfo.getAttribute("language");
		}
		return new LogInfo().getLogInfoSQL(beforeMap, this.getAttributesMap(),
				language, this.DbActionMode);
	}
