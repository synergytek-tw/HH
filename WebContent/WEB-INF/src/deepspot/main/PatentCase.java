...
public class PatentCase extends DraftMasterBean {
    ...
    public Field AgentIssueDate;
    public void setAgentIssueDate(String AgentIssueDate){this.AgentIssueDate.setValue(AgentIssueDate);}
    public String getAgentIssueDate(){return this.AgentIssueDate.getValue();}
    
    public Field AgentIssueNo;
    public void setAgentIssueNo(String AgentIssueNo){this.AgentIssueNo.setValue(AgentIssueNo);}
    public String getAgentIssueNo(){return this.AgentIssueNo.getValue();}
    ...
    public void init() {
        ...
        this.AgentIssueDate = new Field("AgentIssueDate", this.getTableName());
        this.AgentIssueDate.setDbType("varchar(10)");

        this.AgentIssueNo = new Field("AgentIssueNo", this.getTableName());
        this.AgentIssueNo.setDbType("varchar(50)");
	...
	public void setDbFields() {
        ...
        this.DbFields.add(this.AgentIssueDate);
        this.DbFields.add(this.AgentIssueNo);
    ...
    public String printMaintainTable() throws Exception {
        ...
        if (this.onMaintainMode()) {
        	this.FamilyOrder.setHtmlType(Cnstnts.HTML_TAG_TEXT);
        	
            this.IdeaId.setHtmlType(Cnstnts.HTML_TAG_TEXT);
            if(this.getIdeaId().trim().length()>0){
            	String BtnHtml = "<input type='button' " +
					"value='"+this.getUserInfo().getMsg("jsp.DraftTechExamine.LinkParentPage")+"' " +
					"onClick='openAppendix(\"Draft.jsp?_flowKey="+this.getIdeaId()+
					"&_getDataFlag=true&_dbActionMode=query&_pageActionMode=maintain" +
					"&_modeLockFlag=true\",\"Draft\",900,800)'/>";
            	this.IdeaId.setAfterText(BtnHtml);
            }
            this.IdeaId.setAnotherField(this.FamilyOrder);
            this.IdeaId.setColspan(1);
            rtnString += this.printHtml(this.IdeaId);
            /*
			...
			*/
            ...
            this.AgentIssueNo.setHtmlType(Cnstnts.HTML_TAG_TEXT);
            
            this.AgentIssueDate.setHtmlType(Cnstnts.HTML_TAG_OPEN_CALENDAR);
            this.AgentIssueDate.setAnotherField(this.AgentIssueNo);
            this.AgentIssueDate.setColspan(1);
            rtnString += this.printHtml(this.AgentIssueDate);
            ...
            if(this.FlowKey.getValue().trim().length()>0){
            	...
            	scriptText = "<script>"+resetBtn+
								...
								"  closeTabFrame();"+
								"  doAjaxRequest('PatentCase','tabText','tabText','detailDiv','innerHTML')"+
								"}"+
								"</script>";
            	...
            	rtnString += "<tr><td colspan=4>"+
            					...
            					"<input type='button' value='"+this.getUserInfo().getMsg("jsp.PatentFeeSchedule.title")+"' "+
            							"id='PatentFeeSchedule' onClick='resetFrame(\"PatentFeeSchedule\")'/>"+
            				 "</td></tr>"+scriptText+
    ...
    public void setSearchListPrpts() {
        ...
        Field[] showField = {this.FlowKey, this.PatentOffice, this.PatentType, 
        		this.Title, this.FeeScheduleDate, this.InCharge, this.Status, this.NotFinished};
    ...
    public List<Object[]> getSearchList() throws Exception {
        ...
        if(this.ClientName.getValue().trim().length()>0){
        	whereStr = "where Client in(select FlowKey from Client where CompanyName like '%"+
        				this.getClientName()+"%') ";
        }
        ...
        String SQL = "select Todos.NotFinished, p.*,IFNULL(c.CompanyName,'') as ClientName "+
        			 "from PatentCase p "+
        	  ...
        	  "left join(select ParentId, count(FlowKey) as NotFinished from Todos "+
        	  			...
        	  			"on p.FlowKey=Todos.ParentId "+
        	  			"where p.Flowkey in(select FlowKey from PatentCase "+whereStr+")";
    ...
    public String getSubmitBtn() throws Exception {
		...
		if(this.getDbActionMode().equals(Cnstnts.PRINT) 
				&& this.getPageActionMode().equals(Cnstnts.SEARCH)){
    ...
    public String printDetailTable() throws Exception {
        ...
        if(this.getTabText().equals("PatentFeeSchedule")){
        	rtnString = new PatentFeeSchedule().getSearchReasult(this.getUserInfo(),this.getFlowKey());
        }
    ...
    public boolean delete() throws Exception {
		...
		sqlList.add("delete from PatentFeeSchedule where ParentId='"+this.getFlowKey()+"'");
