...
public class TrademarkInventor extends MultiDetail {

    public TrademarkInventor(){this.init();}

    public TrademarkInventor(String FlowKey) {
        this.init();
    ...
    public void init() {
        super.init();
        this.setProgramId("jsp.TrademarkInventor");
        this.setTableName("TrademarkInventor");
    ...
    public String printMaintainTable() throws Exception {
        ...
        if (this.onMaintainMode()) {
    		...
    		String SQL="select * from TrademarkCase where FlowKey='"+this.getParentId() + "'";
    		
    		Field Inventor;
    		Inventor= new Field("Inventor","patentcase");
    		Inventor.setDbType("nvarchar,100");
    		Inventor.setHtmlType(Cnstnts.HTML_TAG_TEXT);
    		String ClientID = Cnstnts.EMPTY_STRING;
    		ClientID= this.getDbUtil().getFieldValue(SQL, Inventor);
    		Inventor.setValue(SQL);
    		
            this.InventorName.setHtmlType(Cnstnts.HTML_TAG_TEXT);
            this.InventorName.setSize(40);
            this.InventorName.setButtonValue(this.getUserInfo().getMsg("jsp.TrademarkInventor.QueryInventor"));
            this.InventorName.setButtonClick("openAppendix(\"QueryForm.jsp?queryClass=TrademarkInventor" +
    ...
    public void setPreObj(UserBean userInfo){
        if (userInfo.getObj(this.getTableName()) != null) {
            TrademarkInventor preObj = (TrademarkInventor) userInfo.getObj(this.getTableName());
	...
	public String printSearchReasult() throws Exception {
		...
		String thText2="<td align='center' width='5%'>"+this.getUserInfo().getMsg("jsp.TrademarkInventor.InventorOrder")+"</td>";
		String thText3="<td align='center' width='10%'>"+this.getUserInfo().getMsg("jsp.TrademarkInventor.InventorType")+"</td>";
		String thText4="<td align='center' width='15%'>"+this.getUserInfo().getMsg("jsp.TrademarkInventor.Inventor")+"</td>";
		...
		for(Iterator itr=findList.iterator();itr.hasNext();i++){
			...
			String onClick = " onClick='openAppendix(\"Employee.jsp?flowKey="+FlowKey+
			  					...
			  					"\",\"TrademarkInventor\",800,600)' style='cursor:hand'";
