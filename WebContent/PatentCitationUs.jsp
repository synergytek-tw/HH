...
<jsp:useBean id="thisObj" scope="page" class="deepspot.main.PatentCitationUs"/>
...
/* ========== orderSequence prorerty access ========== */
  ...
  if(orderSequence.trim().length()==0){
    orderSequence=userInfo.getAction("PatentCitationUs"+"OrderSequence");
  }
  if(orderSequence==null){
    orderSequence=Cnstnts.DESC;
  }
  thisObj.setOrderSequence(orderSequence);
  userInfo.setAction("PatentCitationUs"+"OrderSequence",orderSequence);
...
/* ========== orderKey prorerty access ========== */
  ...
  if(orderKey.trim().length()==0){
    orderKey=userInfo.getAction("PatentCitationUs"+"OrderKey");
  }
  if(orderKey==null){
    orderKey="flowKey";
  }
  thisObj.setOrderKey(orderKey);
  userInfo.setAction("PatentCitationUs"+"OrderKey",orderKey);
...
/* ========== dbActionMode prorerty access ========== */
  ...
  if(dbActionMode.trim().length()==0){
    dbActionMode=userInfo.getAction("PatentCitationUs");
  }
  if(dbActionMode==null){
  ...
  if(!dbActionMode.equals("sign") && !dbActionMode.equals("close") && !dbActionMode.equals("ending")){
    userInfo.setAction("PatentCitationUs",dbActionMode);
  }
  ...
  if(pageActionMode.trim().length()==0){
    pageActionMode=userInfo.getAction("PatentCitationUs"+Cnstnts.PAGE_ACTION_MODE);
  }
  if(pageActionMode==null){
    pageActionMode=Cnstnts.SEARCH;
  }
  thisObj.setPageActionMode(pageActionMode);
  userInfo.setAction("PatentCitationUs"+Cnstnts.PAGE_ACTION_MODE,pageActionMode);
  ...
  userInfo.setObj("PatentCitationUs",thisObj);
%>
...
<%if("true".equals(userInfo.getAttribute("PatentCitationUsCloseFrame"))){
    userInfo.setAttribute("PatentCitationUsCloseFrame","false");%>
    <script>
      parent.resetFrame('PatentCitationUs');
    </script>
<%}%>