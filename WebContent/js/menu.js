...
function Menu()
{
	...
	if (ie) this.menuFont = "12px Arial"; //default font settings. Don't change. Instead, modify stylesheet in sample.htm
	if (n)  this.menuFont = "12px Verdana";
	...
	HTMLstr += "\n";
//	if (ie||ns6) HTMLstr += "<div id='MainTable' style='position:absolute;top:0;left:0;right:0;'>\n";
//	if (n)  HTMLstr += "<layer name='MainTable'>\n";
	HTMLstr += "<table width='600' align='left' bgcolor='"+this.bgColor+"' border='"+this.mainPaneBorder+"'>\n";
	HTMLstr += "<tr>";
	if (n) HTMLstr += "<td>&nbsp;";
...
function addSubItem(menu1, menu2, idParent, text, hint, location, linktarget)
{
	var MENUitem = "";
	...
	if (HTMLstr.indexOf(Lookup) == -1)
	{
		...
		if (ie||ns6)
		{
			...
			MENUitem += "<table border='"+this.subMenuPaneBorder+"' bgcolor='"+submenucolor+"' width="+this.subMenuPaneWidth+">\n";
			MENUitem += "<!-- NEXT ITEM OF SUB MENU "+ idParent +" -->\n";
			MENUitem += "</table>\n";
			MENUitem += "</div>\n";
			MENUitem += "\n";
	...
	if (ie||ns6) MENUitem = "<tr><td><a class=clsMenuItemIE title='"+hint+"' href='"+location+"?menu1=" + menu1 + "&menu2=" + menu2 + "' target='"+linktarget+"'>&nbsp;&nbsp;"+text+"</a><br></td></tr>\n";
	MENUitem += Lookup;
...
//		submenu.top  = calculateSumOffset(menu, 'offsetTop') + 50;
		submenu.top  = menu.style.top+108;
		submenu.visibility = fShow;
		if (lastMenu != null && lastMenu != submenu) hideAll();
